<template>
  <div v-if="showInstallPrompt" class="fixed bottom-4 left-4 right-4 bg-indigo-600 text-white p-4 rounded-lg shadow-lg z-50 flex items-center justify-between safe-area-bottom">
    <div class="flex items-center space-x-3">
      ...
      <div>
        <p class="font-medium text-sm sm:text-base">모임 매니저 설치</p>
        <p class="text-xs sm:text-sm opacity-90">홈 화면에서 바로 실행하세요!</p>
      </div>
    </div>
    <div class="flex space-x-2 ml-4">
      <button @click="installApp" class="bg-white text-indigo-600 px-3 py-1 rounded text-sm font-medium">
        설치
      </button>
      <button @click="dismissInstall" class="text-white opacity-75 hover:opacity-100">
