      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          <div class="flex items-center space-x-3">
            <img src="/icons/logo-transparent.svg" alt="한입 모임 로고" class="h-8 w-8">
          ...
          <div class="flex items-center space-x-1 sm:space-x-4">
            ...
            <button class="btn-danger px-1 sm:px-4 py-1 sm:py-2 text-sm" @click="logout">
      ...
      <div class="px-4 py-6 sm:px-0">
        ...
        <div v-else class="space-y-6">
          ...
          >
            ...
            <div
              v-if="sortedMeetups.length === 0"
              class="px-4 py-10 sm:px-6 text-center border-t border-beige-300 dark:border-neutral-800 bg-white dark:bg-neutral-900"
            >
              <svg
              ...
              <p class="mt-4 text-sm text-neutral-600 dark:text-neutral-300">
                {{ selectedMonthLabel }}에 개설한 모임이 없습니다.
              </p>
              <p class="mt-2 text-xs text-neutral-500 dark:text-neutral-400">
                월 선택을 바꿔 다른 기간의 모임을 확인하거나 새 모임을 만들어보세요.
              </p>
              <div class="mt-6">
                <router-link
                  to="/create-meetup"
            ...
            <template v-else>
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="pl-15 space-y-3">
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <div class="flex-1">
                          <div
                            v-if="meetup.hashtags_list && meetup.hashtags_list.length > 0"
                        ...
                        <div class="flex items-stretch space-x-1 ml-4">
                          ...
                          <router-link
                            :to="`/meetup/${meetup.id}/tasks/manage`"
                            class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md transition-colors text-purple-700 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900 dark:text-purple-300 dark:hover:bg-purple-800"
                            title="과제 관리"
                          >
                            <svg
                              class="w-3 h-3 mr-1"
              ...
              <div class="sm:hidden space-y-4 p-4">
                ...
                >
                  ...
                  <div class="grid grid-cols-4 gap-2 pt-2 border-t border-beige-300 dark:border-neutral-700">
                    <button
                      class="flex items-center justify-center px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-blue-700 bg-blue-100 hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800"
                      @click="editMeetup(meetup)"
                    >
                      수정
                    </button>
                    <button
                      class="flex items-center justify-center px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-orange-700 bg-orange-100 hover:bg-orange-200 dark:bg-orange-900 dark:text-orange-300 dark:hover:bg-orange-800"
                      @click="openManageParticipants(meetup)"
                    >
                      참가자
                    </button>
                    <router-link
                      :to="`/meetup/${meetup.id}/tasks/manage`"
                      class="flex items-center justify-center px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-purple-700 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900 dark:text-purple-300 dark:hover:bg-purple-800"
                    >
                      과제
                    </router-link>
                    <button
                      class="flex items-center justify-center px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-red-700 bg-red-100 hover:bg-red-200 dark:bg-red-900 dark:text-red-300 dark:hover:bg-red-800"
                      @click="deleteMeetup(meetup.id)"
                    >
                      삭제
                    </button>
        ...
        >
          ...
          <div
            v-if="!loadingRegistered && !loadingWaitlist && allMyMeetups.length > 0"
    ...
    >
      <div class="bg-beige-50 dark:bg-neutral-800 rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-hidden">
        ...
        <div class="relative p-6 border-b border-neutral-200 dark:border-neutral-600 bg-beige-200 dark:bg-neutral-700">
          <!-- Close Button -->
          <button class="absolute top-4 right-4 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 transition-colors" @click="closeEditModal">
            <svg
              class="w-5 h-5"
              fill="none"
              stroke="currentColor"
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          <form class="space-y-4" @submit.prevent="updateMeetup">
            <div>
      ...
      <div class="bg-beige-50 dark:bg-neutral-800 rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          <div class="mb-6">
            ...
            <form class="space-y-3" @submit.prevent="addParticipantManually">
          ...
          <div class="bg-white dark:bg-neutral-700 rounded-lg p-4 mt-4">
            ...
            <form class="space-y-3" @submit.prevent="sendNotificationToParticipants">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  제목 *
                </label>
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          >
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center space-x-3">
                ...
                >
                  <svg
                    v-if="selectedNotification.notification_type === 'waitlist_promotion'"
                    class="w-5 h-5 text-neutral-700 dark:text-neutral-300"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                ...
                <div>
                  ...
                  <div class="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    <span
                      v-if="selectedNotification.meetup_name"
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-lg p-4 border border-gray-200 dark:border-neutral-600 relative group">
              ...
              <div
                class="text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line pr-8"
          ...
          <div class="p-6">
            ...
            <div v-if="loadingNotifications" class="py-12 text-center">
              <svg
                class="animate-spin -ml-1 mr-3 h-12 w-12 text-neutral-500 mx-auto"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
              >
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-600 bg-white dark:bg-neutral-800">
                  ...
                  >
                    ...
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                      <div class="flex items-center">
                        <div v-if="!notification.is_read" class="w-2 h-2 bg-neutral-500 rounded-full mr-3" />
                        <div v-else class="w-2 h-2 bg-gray-300 dark:bg-gray-600 rounded-full mr-3" />
                        <span
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm">
                      <div class="flex items-center">
                        ...
                        >
                          <svg
                            v-if="notification.notification_type === 'waitlist_promotion'"
                            class="w-4 h-4 text-neutral-700 dark:text-neutral-300"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                    ...
                    <td class="px-3 py-4 text-sm text-gray-900 dark:text-white">
                      <div class="font-medium truncate max-w-xs" :title="notification.title">
                        {{ notification.title }}
                      </div>
                    </td>
                    <!-- Meetup -->
            ...
            <!-- Pagination -->
            <div
              v-if="notificationPagination.total > notificationPagination.per_page"
            ...
            >
              <div class="flex-1 flex justify-between sm:hidden">
                ...
                <button
                  ...
                  @click="goToPage(notificationPagination.current_page - 1)"
                >
                  이전
                </button>
                <button
...
export default {
  ...
  setup() {
    ...
    const meetupsLoaded = ref(false)
    const refreshingMeetups = ref(false)
    ...
    const loadMeetups = async () => {
      ...
      try {
        ...
        loading.value = false
        refreshingMeetups.value = false
        meetupsLoaded.value = true
      }
    }
